        ...
        for (const [i, fila] of filas.entries()) {
            try {
                ...
                if (!ruc || !tipo || !dniAsesor || !fechaStr) {
                    errores.push({ fila: i+2, error: 'Faltan campos obligatorios' });
                    continue;
                }

                if (!TIPOS_VALIDOS.includes(tipo)) {
                    errores.push({ fila: i+2, error: `Tipo inválido: ${tipo}` });
                    continue;
                }
                ...
                if (isNaN(fecha)) { errores.push({ fila: i+2, error: `Fecha inválida: ${fechaStr}` }); continue; }

                // Buscar empresa en BD General
                ...
                // Construir oportunidad si es interesado
