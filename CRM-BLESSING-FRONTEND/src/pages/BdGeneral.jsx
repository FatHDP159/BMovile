...
const BdGeneral = () => {
    ...
    const handleAsignar = async () => {
        try {
            await api.patch(`/bd-general/${empresaSeleccionada._id}/asignar`, {
                id_asesor: asesorSeleccionado,
            });
    ...
    const handleDesasignar = async (id) => {
        ...
        try {
            await api.patch(`/bd-general/${id}/desasignar`);
            cargarEmpresas(page);
        } catch (err) {
            console.error(err);
        }
    ...
    const handleDesasignarTodo = async (id_asesor, nombre) => {
        ...
        try {
            await api.patch(`/bd-general/desasignar-todo/${id_asesor}`);
            cargarEmpresas(page);
        } catch (err) {
            console.error(err);
        }
    ...
    const handleEliminar = async (id) => {
        ...
        try {
            await api.delete(`/bd-general/${id}`);
            cargarEmpresas(page);
        } catch (err) {
            console.error(err);
        }
    ...
    const handleExportar = async () => {
        try {
            // Traer todos los datos con los filtros actuales sin paginación
            const res = await api.get('/bd-general/buscar', {
                params: {
                    busqueda,
                    estado: filtroEstado,
            ...
            datos.forEach(e => {
                ...
                filas.push([
                    ...
                    e.estado_base || '',
                    e.asignacion?.id_asesor?.nombre_user || '',
                    contacto.nombre || '',
                    contacto.dni || '',
                    contacto.cargo || '',
    ...
    return (
        <div>
            ...
            <div className="search-bar">
                ...
                <input
                    className="search-input"
                    placeholder="Buscar consultor SF..."
                ...
                <input
                    ...
                    title="Fecha asignación SF"
                    value={filtroFechaAsig}
                    onChange={(e) => setFiltroFechaAsig(e.target.value)}
                />
                <input
                    type="date"
                    className="filter-select"
                    title="Fecha desasignación SF"
                    value={filtroFechaDesasig}
                    onChange={(e) => setFiltroFechaDesasig(e.target.value)}
                />
            </div>
                            ...
                            <tbody>
                                {empresas.map((e) => (
                                    <tr key={e._id}>
                                        ...
                                        <td>
                                            <button
                                                className="btn-contactos"
                                                onClick={() => { setContactosEmpresa(e); setShowModalContactos(true); }}
                                            >
                                        ...
                                        <td>
                                            <span className={`badge badge-${e.estado_base}`}>
                                                {e.estado_base}
                                            </span>
                                        </td>
                                        <td>
                                            {e.asignacion?.id_asesor ? (
                                                <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                                    ...
                                                    <button
                                                        className="btn-estado btn-desasignar"
                                                        style={{ fontSize: '10px', padding: '3px 8px' }}
                                                        onClick={() => handleDesasignarTodo(
                                                            e.asignacion.id_asesor._id,
                                                            e.asignacion.id_asesor.nombre_user
                                                        )}
                                        ...
                                        <td style={{ display: 'flex', gap: '6px' }}>
                                            {e.estado_base === 'disponible' && (
                                                <button
                                                    className="btn-estado btn-asignar"
                                                    onClick={() => { setEmpresaSeleccionada(e); setShowModalAsignar(true); }}
                                                >
                                            ...
                                            {(e.estado_base === 'asignada' || e.estado_base === 'trabajada') && (
                                                <button
                                                    className="btn-estado btn-desasignar"
                                                    onClick={() => handleDesasignar(e._id)}
                                                >
                    ...
                    <div className="modal">
                        ...
                        <div className="form-field">
                            ...
                            <select className="form-input" value={asesorSeleccionado} onChange={(e) => setAsesorSeleccionado(e.target.value)}>
                                ...
                                {usuarios.map((u) => (
                                    <option key={u._id} value={u._id}>{u.nombre_user} - {u.dni_user}</option>
                                ))}
                            </select>
                        </div>
            ...
            {showModalMasivo && (
                <div className="modal-overlay">
                    <div className="modal">
                        ...
                        <div className="form-field">
                            <label>Líneas mínimas (opcional)</label>
                            <input
                                type="number"
                                className="form-input"
                                placeholder="Ej: 5"
                                value={formMasivo.lineas_min}
                                onChange={(e) => setFormMasivo({ ...formMasivo, lineas_min: e.target.value })}
                                min="0"
                            />
                        </div>
                        <div className="form-field">
                            <label>Líneas máximas (opcional)</label>
                            <input
                                type="number"
                                className="form-input"
                                placeholder="Ej: 30"
                                value={formMasivo.lineas_max}
                                onChange={(e) => setFormMasivo({ ...formMasivo, lineas_max: e.target.value })}
                                min="0"
                            />
                        </div>
                        <div className="modal-actions">
