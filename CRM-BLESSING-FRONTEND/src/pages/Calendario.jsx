    ...
    const handleGuardar = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                recordatorio: form.recordatorio || null,
                cliente: clienteSel ? {
                    id_gestion: clienteSel._id,
                    ruc: clienteSel.ruc,
                    razon_social: clienteSel.razon_social,
                } : {},
            };
            if (actividad?._id) {
                await api.put(`/actividades/${actividad._id}`, payload);
            } else {
...
const ActividadCard = ({ act, onEditar, onCambiarEstado, onEliminar, compacto = false, esPasada }) => {
    const tipo = TIPOS[act.tipo];
    const prio = PRIORIDADES[act.prioridad];
    const vencida = act.estado === 'pendiente' && esPasada && esPasada(act.fecha, act.hora);

    return (
        <div className={`act-card ${compacto ? 'compacto' : ''} ${vencida ? 'vencida' : ''} estado-${act.estado}`}>
...
const Calendario = () => {
    ...
    // Actualiza el reloj cada minuto para detectar vencimientos en tiempo real
    useEffect(() => {
    ...
    const cargar = useCallback(async () => {
        try {
            ...
            const [semRes, hoyRes, pendRes, funnelRes] = await Promise.all([
    ...
    const handleCambiarEstado = async (id, estado) => {
        try {
            await api.put(`/actividades/${id}`, { estado });
            cargar();
    ...
    const pendientesVencidas = pendientes.filter(a => esPasada(a.fecha, a.hora));
    const pendientesProximas = pendientes.filter(a => !esPasada(a.fecha, a.hora));
        ...
        <div className="calendario-page">
            ...
            <div className="panel-hoy">
                ...
                <div className="panel-hoy-lista">
                    {actHoy.length === 0 ? (
                        ...
                        actHoy.map(a => (
                            <ActividadCard key={a._id} act={a} compacto esPasada={esPasada}
            ...
            <div className="cal-container">
                <div className="cal-nav">
                    <button className="btn-secondary" onClick={() => setSemanaOffset(o => o - 1)}>
                    ...
                    {semanaOffset !== 0 && (
                        <button className="btn-secondary" style={{ fontSize: 11 }} onClick={() => setSemanaOffset(0)}>Hoy</button>
                    )}
                </div>
                                        ...
                                        {acts.map(a => {
                                            ...
                                            return (
                                                <div key={a._id} className="cal-act-bloque"
                                                    style={{ background: tipo.color, borderLeft: `3px solid ${tipo.text}` }}
                                                    onClick={() => { setModalAct(a); setModalAbierto(true); }}
                                                    title={a.titulo}
                                                >
                    ...
                    <div className="pendientes-seccion">
                        ...
                        <div className="pendientes-lista">
                            {pendientesVencidas.map(a => (
                                <ActividadCard key={a._id} act={a} esPasada={esPasada}
                                    onEditar={act => { setModalAct(act); setModalAbierto(true); }}
                                    onCambiarEstado={handleCambiarEstado}
                                    onEliminar={handleEliminar}
                                />
