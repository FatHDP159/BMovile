...
const EstadoOpoBadge = ({ estado }) => {
    ...
    if (!e) return null;
    return <span className="estado-opo-badge" style={{ background: e.color, color: e.text }}>{estado}</span>;
};

// ── Modal Ficha ───────────────────────────────────────────────────────────────
const ModalFicha = ({ ficha: fichaInicial, onClose }) => {
                ...
                <div style={{ marginBottom: 16 }}>
                    ...
                    {opoMasAvanzada() && (
                        <div style={{ marginTop: 8 }}>
                            <EstadoOpoBadge estado={opoMasAvanzada()?.estado} />
                        </div>
                    )}
                </div>
...
const GestionesSupervisor = () => {
    ...
    const cargar = useCallback(async (p = 1) => {
        ...
        try {
            const res = await api.get('/ficha-gestion', {
                params: { busqueda, asesor: filtroAsesor, estado_general: filtroEstado, page: p, limit: 50 }
            });
    ...
    }, [busqueda, filtroAsesor, filtroEstado]);
